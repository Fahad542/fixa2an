...
function Navbar() {
	...
	const shouldUseWhiteNavbar = !isHomePage || isScrolled
	...
	return (
		<header 
			className={`fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300 ${
				shouldUseWhiteNavbar ? 'bg-white shadow-lg' : 'bg-transparent'
			}`}
			style={{ 
		...
		>
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-2 sm:pb-0">
						...
						>
							<img 
								...
								className={`h-10 sm:h-14 md:h-16 w-auto object-contain transition-all duration-300 ${
									shouldUseWhiteNavbar ? '' : 'brightness-0 invert'
								}`}
							/>
						</Link>
						{user?.role === 'ADMIN' && (
						...
						{!loading && user ? (
										...
										className={`relative transition-all duration-300 px-4 py-2.5 rounded-lg ${
											isActive('/admin') 
												? shouldUseWhiteNavbar 
													? 'text-[#1C3F94] font-semibold bg-blue-50 shadow-sm' 
													: 'text-white font-semibold bg-white/25 shadow-md backdrop-blur-sm'
												: shouldUseWhiteNavbar 
													? 'text-gray-600 hover:text-[#1C3F94] hover:bg-gray-50 hover:shadow-sm' 
							...
							<>
								...
								<button
									...
									className={`relative transition-all duration-300 px-4 py-2 rounded-lg font-medium ${
										shouldUseWhiteNavbar 
											? 'text-white bg-[#1C3F94] hover:bg-[#2a5bb8] shadow-md hover:shadow-lg' 
											: 'text-[#1C3F94] bg-white hover:bg-gray-50 shadow-lg hover:shadow-xl'
									}`}
								>
							...
							{user ? (
									...
									{user.role === 'ADMIN' && (
										<Link
											...
											className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${
												isActive('/admin') 
													? 'text-[#1C3F94] font-semibold bg-blue-50 shadow-sm'
													: 'text-gray-700 hover:text-[#1C3F94] hover:bg-gray-50'
											}`}
											onClick={() => setMobileMenuOpen(false)}
										>
											<div className={`w-2 h-2 rounded-full ${isActive('/admin') ? 'bg-[#1C3F94]' : 'bg-gray-300'}`}></div>
									...
									{user.role === 'CUSTOMER' && (
										<>
											<Link
												...
												onClick={() => setMobileMenuOpen(false)}
											>
												<div className={`w-2 h-2 rounded-full ${isActive('/my-cases') ? 'bg-[#1C3F94]' : 'bg-gray-300'}`}></div>
											...
											<Link
												...
												onClick={() => setMobileMenuOpen(false)}
											>
												<div className={`w-2 h-2 rounded-full ${isActive('/upload') ? 'bg-[#1C3F94]' : 'bg-gray-300'}`}></div>
											...
											<Link
												...
												onClick={() => setMobileMenuOpen(false)}
											>
												<div className={`w-2 h-2 rounded-full ${isActive('/profile') ? 'bg-[#1C3F94]' : 'bg-gray-300'}`}></div>
									...
									{user.role === 'WORKSHOP' && (
										<>
											<Link
												...
												onClick={() => setMobileMenuOpen(false)}
											>
												<div className={`w-2 h-2 rounded-full ${isActive('/workshop/requests') ? 'bg-[#1C3F94]' : 'bg-gray-300'}`}></div>
											...
											<Link
												...
												onClick={() => setMobileMenuOpen(false)}
											>
												<div className={`w-2 h-2 rounded-full ${isActive('/workshop/proposals') ? 'bg-[#1C3F94]' : 'bg-gray-300'}`}></div>
											...
											<Link
												...
												onClick={() => setMobileMenuOpen(false)}
											>
												<div className={`w-2 h-2 rounded-full ${isActive('/workshop/contracts') ? 'bg-[#1C3F94]' : 'bg-gray-300'}`}></div>
											...
											<Link
												...
												onClick={() => setMobileMenuOpen(false)}
											>
												<div className={`w-2 h-2 rounded-full ${isActive('/workshop/profile') ? 'bg-[#1C3F94]' : 'bg-gray-300'}`}></div>
												<span>{t('navigation.profile') || 'Profile'}</span>
											</Link>
										</>
									)}
								...
								<>
									<Link
										...
										onClick={() => setMobileMenuOpen(false)}
									>
										<div className={`w-2 h-2 rounded-full ${isActive('/auth/signin') ? 'bg-[#1C3F94]' : 'bg-gray-300'}`}></div>
									...
									<button
										...
										className="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 text-left w-full text-white bg-[#1C3F94] hover:bg-[#2a5bb8] font-medium shadow-sm"
									>
