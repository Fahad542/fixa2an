	...
	const fetchRequest = async () => {
		try {
			...
			if (response.data) {
				...
				const foundRequest = requests.find((r) => (r._id || r.id) === requestId)
				if (foundRequest) {
					setRequest(foundRequest)
					
					// Check if this workshop has already submitted an offer
					const existingOffers = foundRequest.offers || []
					if (existingOffers.length > 0) {
						// The existingOffers array contains offers from this workshop
						// Use the first offer (there should only be one per workshop per request)
						const existingOfferId = existingOffers[0]._id || existingOffers[0].id
						
						// Fetch full offer details
						try {
							const offersResponse = await offersAPI.getByRequest(requestId)
							if (offersResponse.data && Array.isArray(offersResponse.data)) {
								// Find the offer by matching the ID
								const workshopOffer = offersResponse.data.find((offer) => {
									const offerId = offer._id || offer.id
									return offerId?.toString() === existingOfferId?.toString()
								})
								
								if (workshopOffer) {
									// Store existing offer
									...
									if (workshopOffer.availableDates) {
										try {
											if (typeof workshopOffer.availableDates === 'string') {
												parsedDates = JSON.parse(workshopOffer.availableDates)
											} else if (Array.isArray(workshopOffer.availableDates)) {
												parsedDates = workshopOffer.availableDates
											}
											...
											console.error('Failed to parse availableDates:', parseError)
											parsedDates = ['']
										}
									}
									
									// Auto-fill form with existing offer data
									setFormData({
							...
							// Continue without pre-filling if offer fetch fails
						}
					}
				} else {
					toast.error(t('errors.request_not_found'))
					navigate('/workshop/requests')
				}
			}
		} catch (error) {
			console.error('Failed to fetch request:', error)
			toast.error(t('errors.fetch_request_failed'))
			navigate('/workshop/requests')
		} finally {
			setLoading(false)
		}
			...
			<div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center pt-20">
				...
				<div className="text-center space-y-4">
					<div className="relative">
						...
						<Send className="w-10 h-10 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" style={{ color: '#1C3F94' }} />
					</div>
			...
			<div className="max-w-5xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 pt-20 sm:pt-24 md:pt-28 pb-12 sm:pb-16">
				...
				<div className="mb-8 sm:mb-10 md:mb-12">
					<div className="flex items-center gap-2 sm:gap-3 mb-2">
						<div className="p-2 sm:p-3 rounded-lg shadow-lg flex-shrink-0" style={{ backgroundColor: '#1C3F94', color: '#FFFFFF' }}>
				...
				<Card className="mb-8 sm:mb-10 md:mb-12 shadow-lg border border-gray-200 bg-white">
					<CardHeader className="pb-4 sm:pb-5 px-5 sm:px-7 pt-5 sm:pt-7">
						<CardTitle className="flex items-center gap-2 sm:gap-3 text-lg sm:text-xl text-gray-900">
							<div className="p-2 sm:p-2.5 rounded-lg shadow-sm flex-shrink-0" style={{ backgroundColor: '#1C3F94', color: '#FFFFFF' }}>
					...
					<CardContent className="pt-0 px-5 sm:px-7 pb-5 sm:pb-7">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 md:gap-10">
							<div>
								<div className="flex items-start gap-3 sm:gap-4">
									<div className="p-2 sm:p-2.5 bg-blue-50 rounded-lg flex-shrink-0">
										<Car className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: '#1C3F94' }} />
									</div>
									<div className="min-w-0 flex-1">
							...
							<div>
								<div className="flex items-start gap-3 sm:gap-4">
									...
									<div className="min-w-0 flex-1">
										...
										<p className="text-xs sm:text-sm text-gray-600 truncate">{customer?.email || 'N/A'}</p>
									</div>
								</div>
							</div>
						</div>
						...
						{request.description && (
							<div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-gray-100">
								<div className="flex items-start gap-3 sm:gap-4">
									<div className="p-2 sm:p-2.5 bg-purple-50 rounded-lg flex-shrink-0">
										<MessageSquare className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: '#1C3F94' }} />
									</div>
				...
				<Card className="shadow-lg border border-gray-200 bg-white">
					<CardHeader className="pb-4 sm:pb-5 px-5 sm:px-7 pt-5 sm:pt-7">
						<CardTitle className="flex items-center gap-2 sm:gap-3 text-lg sm:text-xl text-gray-900">
							<div className="p-2 sm:p-2.5 rounded-lg shadow-sm flex-shrink-0" style={{ backgroundColor: '#34C759', color: '#FFFFFF' }}>
						...
						<form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
								...
								<div className="space-y-2 sm:space-y-3">
									<Label htmlFor="estimatedDuration" className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-700">
										<div className="p-1 sm:p-1.5 bg-blue-50 rounded-md flex-shrink-0">
											<Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: '#1C3F94' }} />
										</div>
							...
							<div className="space-y-2 sm:space-y-3">
								<Label htmlFor="warranty" className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-700">
									<div className="p-1 sm:p-1.5 bg-amber-50 rounded-md flex-shrink-0">
										<Shield className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: '#1C3F94' }} />
									</div>
							...
							<div className="space-y-2 sm:space-y-3">
								<Label className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-700">
									<div className="p-1 sm:p-1.5 bg-indigo-50 rounded-md flex-shrink-0">
										<Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: '#1C3F94' }} />
									</div>
									<span>
							...
							<div className="space-y-2 sm:space-y-3">
								<Label htmlFor="note" className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-gray-700">
									<div className="p-1 sm:p-1.5 bg-purple-50 rounded-md flex-shrink-0">
										<FileText className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: '#1C3F94' }} />
									</div>
